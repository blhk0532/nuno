<div>
            ...
            window.ChatDrawer = () => {
                return {
                    ...
                    closingModal(eventName) {
                        const drawerComponents = this.$wire.get('drawerComponents') || {};
                        const activeComponent = drawerComponents[this.activeDrawerComponent];
                    ...
                    init() {
                        ...
                        // Listen for openChatDrawer event from Alpine.js $dispatch
                        this.$watch('$wire.drawerComponents', () => {
                            // React to drawer components changes
                        });
                    },
                    destroy() {
    ...
    x-data="ChatDrawer()" x-on:close.stop="setShowPropertyTo(false)"
         ...
         x-show="show"
         class="absolute bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] dark:text-white opacity-100 inset-0 z-50 h-full overflow-y-auto wc-scrollbar-theme" style="display: none;"
         aria-modal="true"
         tabindex="0"
