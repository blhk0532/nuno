...
<header class=" sticky top-0 bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] z-10 p-2">
    <div class="flex items-center pb-2">
        ...
        <h3  class="text-sm mx-auto font-semibold "  ><span>{{__('filament-wirechat::chat.group.add_members.heading.label')}} </span> {{$newTotalCount}} / {{$maxGroupMembers}}</h3>
        ...
        <button
            ...
            class="">
            {{__('filament-wirechat::chat.group.add_members.actions.save.label')}}
        </button>
    ...
    <div
      ...
      "
       ...
       <span x-transition x-show="showError">
        {{__('filament-wirechat::chat.group.add_members.messages.members_limit_error',['count'=>$maxGroupMembers])}}
       </span>
    </div>
                ...
                @foreach ($selectedMembers as $key => $member)
                    <li class="flex items-center text-nowrap min-w-fit px-2 py-1 text-sm font-medium text-gray-800 bg-[var(--wc-light-secondary)] rounded-sm dark:bg-[var(--wc-dark-secondary)] dark:text-gray-300"
                        ...
                        <button type="button"
                            ...
                            class="flex items-center p-1 ms-2 text-sm text-gray-400 bg-transparent rounded-xs hover:bg-[var(--wc-light-secondary)] hover:text-gray-900 dark:hover:bg-[var(--wc-dark-secondary)] dark:hover:text-gray-300"
                            aria-label="Remove">
                           ...
                           <div @class(['opacity-70' => $isAlreadyAParticipant]) >
                            ...
                            @class(['transition-all truncate', 'group-hover:underline ' => !$isAlreadyAParticipant])>
                             ...
                             @class(['text-gray-600 dark:text-gray-400 text-sm'])>
                                @if ($isAlreadyAParticipant)
                                {{__('filament-wirechat::chat.group.add_members.messages.member_already_exists')}}
                                @endif
                             </span>
                           </div>
