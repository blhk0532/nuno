...
@php
// For Filament, we'll use a default primary color
$primaryColor = 'oklch(0.623 0.214 259.815)';
$hasEmojiPicker = true; // Enable emoji picker to match preview
@endphp
...
"
    class="w-full h-full transition wc-scrollbar-theme bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] overflow-hidden relative flex flex-col" style="contain:content">
    
    {{-- Drawer positioned relative to chat container --}}
    ...
    <div class="flex flex-col h-full relative overflow-hidden">
        ...
        {{-- ---------- --}}
        <div class="shrink-0">
            @include('filament-wirechat::livewire.chat.partials.header', [ 'conversation' => $conversation, 'receiver' => $receiver])
        </div>
        {{-- ---------- --}}
        {{-- -Body----- --}}
        {{-- ---------- --}}
        <div class="flex-1 min-h-0 overflow-hidden">
            @include('filament-wirechat::livewire.chat.partials.body', [ 'conversation' => $conversation, 'authParticipant' => $authParticipant, 'loadedMessages' => $loadedMessages, 'isPrivate' => $conversation->isPrivate(), 'isGroup' => $conversation->isGroup(), 'receiver' => $receiver])
        </div>
        {{-- ---------- --}}
        {{-- -Footer--- --}}
        {{-- ---------- --}}
        <div class="shrink-0">
            @include('filament-wirechat::livewire.chat.partials.footer', [ 'conversation' => $conversation, 'authParticipant' => $authParticipant, 'media' => $media, 'files' => $files, 'replyMessage' => $replyMessage])
        </div>
