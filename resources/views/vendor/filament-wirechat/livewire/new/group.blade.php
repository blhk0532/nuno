            ...
            <form wire:submit="validateDetails()" class="flex flex-col  h-full p-4">
                    ...
                    <div class="flex gap-10 w-full">
                        ...
                        @if ($photo)
                            <div class="relative  w-28 h-28 overflow-clip rounded-full">
                                <x-filament-wirechat::avatar :src="$photo->temporaryUrl()" class="w-28 h-28" />
                                <button
                                    type="button"
                                    class="bottom-0 inset-x-0 bg-[var(--wc-light-secondary)]/40 dark:bg-[var(--wc-dark-secondary)]/40 text-red-800 dark:text-red-400 flex items-center justify-center  absolute "
                        ...
                        @else
                            <label class="cursor-pointer">
                                <x-filament-wirechat::avatar wire:loading.class="animate-pulse" wire:target="photo" :group="true" class="w-28 h-28" />
                        ...
                        <div class=" my-auto">
                            ...
                            <label for="name">@lang('filament-wirechat::new.group.inputs.name.label')</label>

                            <input id='name' type="text" wire:model='name' autofocus placeholder="{{__('filament-wirechat::new.group.inputs.name.placeholder') }}"
                ...
                <main class="my-5">
                    <div class=" my-auto flex flex-col gap-y-2">
                        ...
                        <label class="my-2" for="description">@lang('filament-wirechat::new.group.inputs.description.label')</label>

                        <textarea id='description' type="text" wire:model='description' placeholder="{{__('filament-wirechat::new.group.inputs.description.placeholder')}}" rows="4"
                ...
                <footer class="flex gap-4 justify-end mt-auto">
                    <x-filament-wirechat::actions.close-modal>
                        <button type="button" dusk="cancel_create_new_group_button"class="font-bold cursor-pointer hover:bg-[var(--wc-light-secondary)] dark:hover:bg-[var(--wc-dark-secondary)] p-3 px-4 rounded-xl ">
                            @lang('filament-wirechat::new.group.actions.cancel.label')
                        </button>
                    </x-filament-wirechat::actions.close-modal>
                    ...
                    <button type="submit" :disabled="!($wire.name?.trim()?.length)" dusk="next_button"
                        ...
                        @lang('filament-wirechat::new.group.actions.next.label')
                    </button>
                </footer>
            </form>
        ...
        <section dusk="add_members_section" x-cloak x-show="$wire.showAddMembers==true"
            ...
            <header class=" sticky top-0 bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] z-10 py-2">
                <div class="flex items-center pb-2">
                    ...
                    <h3 class="text-sm mx-auto font-semibold "><span>@lang('filament-wirechat::new.group.labels.add_members')</span> {{count($selectedMembers)}} / {{$maxGroupMembers}}</h3>
                    ...
                    <button
                        ...
                        @lang('filament-wirechat::new.group.actions.create.label')
                ...
                <div
                 ...
                 class="text-red-500 text-sm mx-auto ">
                   <span x-transition x-show="showError">
                    @lang('filament-wirechat::new.group.messages.members_limit_error',['count'=>$maxGroupMembers])
                   </span>
                </div>
                {{-- Search input --}}
                <section class="flex flex-wrap items-center px-0 border-b border-[var(--wc-light-secondary)] dark:border-[var(--wc-dark-secondary)]">
                    <input type="search" id="users-search-field" wire:model.live.debounce='search' autocomplete="off"
                        placeholder="{{__('filament-wirechat::new.group.inputs.search.placeholder')}}"
                        class="wc-input w-full border-0 w-auto h-9 px-3 dark:bg-[var(--wc-dark-primary)] outline-hidden focus:outline-hidden bg-[var(--wc-light-primary)] bg-none rounded-lg focus:ring-0 hover:ring-0">
                </section>
            ...
            <div class="relative w-full">
                ...
                <section class="my-4 grid">
                            ...
                            @foreach ($users as $key => $user)
                                <li class="flex cursor-pointer group gap-2 items-center p-2">
                                    ...
                                    <label
                                        ...
                                        <x-filament-wirechat::avatar  src="{{ $user['wirechat_avatar_url'] }}" class="w-10 h-10" />
                    ...
                    @else
                        @if (!empty($search))
                        <span class="m-auto">@lang('filament-wirechat::new.group.messages.empty_search_result')</span>
                        @endif
                    @endif
                </section>
            </div>
