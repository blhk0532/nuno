...
export function EventCalendarDemo2() {
  ...
  // Load second technician quickly (doesn't block dropdown data)
  useEffect(() => {
  ...
  const handleDateSelect = useCallback(
    (info: any) => {
      ...
      setData((currentData) => ({
        ...
        service_user_id: info.resource?.id || secondTechnician?.id || '',
      }));

      setIsEditMode(false);
      setShowBookingModal(true);
    },
    [setData, secondTechnician],
  );
    ...
    <>
      {!secondTechnician ? (
        ...
        <EventCalendar
          ...
          events={`/api/calendar/bookings?service_user_id=${secondTechnician.id}`}
          ...
          weekends={true}
          addButton={{ text: `Boka ${secondTechnician.title}`, click() { setIsEditMode(false); setSelectedSlot({ start: new Date(), end: new Date(Date.now() + 60 * 60 * 1000), allDay: false }); setShowBookingModal(true); } }}
        />
      )}
        ...
        <div className="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Starttid</label>
                  <input type="time" value={data.start_time} onChange={(e) => setData('start_time', e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" required />
                </div>

                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Sluttid</label>
                  <input type="time" value={data.end_time} onChange={(e) => setData('end_time', e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" required />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Kund</label>

                  <div className="flex items-center space-x-3">
                    <select value={data.booking_client_id} onChange={(e) => setData('booking_client_id', e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                      <option value="">Välj kund</option>
                      {dropdownData.clients.map((client: any) => (<option key={client.id} value={client.id}>{client.name}</option>))}
                    </select>
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Tjänst</label>
                  <select value={data.service_id} onChange={(e) => setData('service_id', e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    <option value="">Välj tjänst</option>
                    {dropdownData.services.map((service: any) => (<option key={service.id} value={service.id}>{service.name}</option>))}
                  </select>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={data.booking_location_id} onChange={(e) => setData('booking_location_id', e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    <option value="">Välj plats</option>
                    {dropdownData.locations.map((location: any) => (<option key={location.id} value={location.id}>{location.name}</option>))}
                  </select>
                </div>

                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Tekniker</label>
                  <select value={data.service_user_id} onChange={(e) => setData('service_user_id', e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    <option value="">Välj tekniker</option>
                    {dropdownData.serviceUsers.map((user: any) => (<option key={user.id} value={user.id}>{user.name}</option>))}
                  </select>
                </div>
              </div>

              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Pris (SEK)</label>
                <input type="number" value={data.total_price} onChange={(e) => setData('total_price', e.target.value)} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" min="0" step="0.01" />
              </div>

              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Interna anteckningar</label>
                <textarea value={data.notes} onChange={(e) => setData('notes', e.target.value)} rows={3} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
              </div>

              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Service anteckningar</label>
                <textarea value={data.service_note} onChange={(e) => setData('service_note', e.target.value)} rows={3} className="w-full rounded-md border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
              </div>

              <div className="flex justify-between pt-4">
                <div>
                  {isEditMode && (<button type="button" onClick={handleDelete} className="rounded-md bg-red-600 px-4 py-2 text-white hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:outline-none">Radera</button>)}
                </div>

                <div className="space-x-3">
                  <button type="button" onClick={() => { setShowBookingModal(false); reset(); setSelectedEvent(null); setSelectedSlot(null); }} className="rounded-md bg-gray-300 px-4 py-2 text-gray-700 hover:bg-gray-400 focus:ring-2 focus:ring-gray-500 focus:outline-none">Avbryt</button>

                  <button type="submit" disabled={processing} className="rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:outline-none disabled:opacity-50">{processing ? 'Sparar...' : isEditMode ? 'Uppdatera' : 'Skapa'}</button>
                </div>
              </div>
            </form>
          </div>
