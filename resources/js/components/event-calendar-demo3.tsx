  ...
  const handleDateSelect = useCallback(
    (info: any) => {
      ...
      setData((currentData) => ({
        ...
        service_user_id: info.resource?.id || thirdTechnician?.id || '',
      }));

      setIsEditMode(false);
      setShowBookingModal(true);
    },
    [setData, thirdTechnician],
  );
    ...
    <>
      {!thirdTechnician ? (
        ...
        <EventCalendar
          ...
          events={`/api/calendar/bookings?service_user_id=${thirdTechnician.id}`}
          ...
          weekends={true}
          addButton={{ text: `Boka ${thirdTechnician.title}`, click() { setIsEditMode(false); setSelectedSlot({ start: new Date(), end: new Date(Date.now() + 60 * 60 * 1000), allDay: false }); setShowBookingModal(true); } }}
        />
      )}
