...
/**
 * Gets the current time in Europe/Stockholm timezone
 * @memberof Timezone
 * @returns {Date} Current time converted to Europe/Stockholm timezone
 *
 * @example
 * const swedenTime = getCurrentTimeInSweden();
 * const currentHour = swedenTime.getHours();
 */
export const getCurrentTimeInSweden = (): Date => {
  return toZonedTime(new Date(), 'Europe/Stockholm');
};
