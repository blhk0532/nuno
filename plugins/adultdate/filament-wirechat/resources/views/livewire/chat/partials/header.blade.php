...
<header
    ...
    <div class="  flex  w-full items-center   px-2 py-3   lg:px-4 gap-2 md:gap-5 ">
        ...
        {{-- Receiver filament-wirechat::Avatar --}}
        <section class="grid grid-cols-12 w-full">
            <div class="shrink-0 col-span-11 w-full truncate overflow-h-hidden relative">

                {{-- Group --}}
                @if ($conversation->isGroup())
                    <x-filament-wirechat::actions.show-group-info conversation="{{ $conversation->id }}"
                        widget="{{ $this->isWidget() }}">
                        <div class="flex items-center gap-2 cursor-pointer ">
                            <x-filament-wirechat::avatar disappearing="{{ $conversation->hasDisappearingTurnedOn() }}"
                    ...
                    </x-filament-wirechat::actions.show-group-info>
                @else
                    {{-- Not Group --}}
                    <x-filament-wirechat::actions.show-chat-info conversation="{{ $conversation->id }}"
                        widget="{{ $this->isWidget() }}">
                        <div class="flex items-center gap-2 cursor-pointer ">
                            <x-filament-wirechat::avatar disappearing="{{ $conversation->hasDisappearingTurnedOn() }}"
                    ...
                    </x-filament-wirechat::actions.show-chat-info>
                @endif
            ...
            <div class="flex gap-2 items-center ml-auto col-span-1">
                <x-filament-wirechat::dropdown align="right" width="48">
                    <x-slot name="trigger">
                    ...
                    <x-slot name="content">
                            ...
                            <x-filament-wirechat::actions.show-group-info conversation="{{ $conversation->id }}"
                                ...
                                <button class="w-full text-start">
                                    <x-filament-wirechat::dropdown-link>
                                        {{ __('filament-wirechat::chat.actions.open_group_info.label') }}
                                    </x-filament-wirechat::dropdown-link>
                                </button>
                            </x-filament-wirechat::actions.show-group-info>
                        @else
                            ...
                            <x-filament-wirechat::actions.show-chat-info conversation="{{ $conversation->id }}"
                                ...
                                <button class="w-full text-start">
                                    <x-filament-wirechat::dropdown-link>
                                        {{ __('filament-wirechat::chat.actions.open_chat_info.label') }}
                                    </x-filament-wirechat::dropdown-link>
                                </button>
                            </x-filament-wirechat::actions.show-chat-info>
                        @endif
                        ...
                        @if ($this->isWidget())
                            <x-filament-wirechat::dropdown-link @click="$dispatch('close-chat',{conversation: {{json_encode($conversation->id)}} })">
                                @lang('filament-wirechat::chat.actions.close_chat.label')
                            </x-filament-wirechat::dropdown-link>
                        @else
                            <x-filament-wirechat::dropdown-link href="{{ $this->chatsRoute()  }}" class="shrink-0">
                                @lang('filament-wirechat::chat.actions.close_chat.label')
                            </x-filament-wirechat::dropdown-link>
                        @endif
                        ...
                        @if (!$conversation->isGroup())
                            <button dusk="clear-chat-action" class="w-full" wire:click="clearConversation"
                                wire:confirm="{{ __('filament-wirechat::chat.actions.clear_chat.confirmation_message') }}">

                                <x-filament-wirechat::dropdown-link>
                                    @lang('filament-wirechat::chat.actions.clear_chat.label')
                                </x-filament-wirechat::dropdown-link>
                            </button>

                           @if(true)
                            <button dusk="delete-chat-action" wire:click="deleteConversation"
                                wire:confirm="{{ __('filament-wirechat::chat.actions.delete_chat.confirmation_message') }}"
                                class="w-full text-start">

                                <x-filament-wirechat::dropdown-link class="text-red-500 dark:text-red-500">
                                    @lang('filament-wirechat::chat.actions.delete_chat.label')
                                </x-filament-wirechat::dropdown-link>
                        ...
                        @if ($conversation->isGroup() && !$this->auth->isOwnerOf($conversation))
                            <button wire:click="exitConversation"
                                wire:confirm="{{ __('filament-wirechat::chat.actions.exit_group.confirmation_message') }}"
                                class="w-full text-start ">

                                <x-filament-wirechat::dropdown-link class="text-red-500 dark:text-gray-500">
                                    @lang('filament-wirechat::chat.actions.exit_group.label')
                                </x-filament-wirechat::dropdown-link>
