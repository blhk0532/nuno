...
<main x-data="{
    ...
    x-cloak
     class='flex flex-col h-full relative gap-2 gap-y-4 p-4 md:p-5 lg:p-8 grow overscroll-contain overflow-y-auto overflow-x-hidden w-full wc-scrollbar-theme'
    ...
    <div x-cloak wire:loading.delay.class.remove="invisible" wire:target="loadMore" class="invisible transition-all duration-300 ">
        <x-filament-wirechat::loading-spin />
    </div>
                ...
                <div class="flex gap-2" wire:key="message-{{ $key }}"  >
                    ...
                    @if (!$belongsToAuth && !$isPrivate)
                        <div @class([
                            ...
                            <x-filament-wirechat::avatar src="{{ $message->sendable?->wirechat_avatar_url ?? null }}" class="h-8 w-8" />
                        </div>
                    @endif
                        ...
                        <div @class([
                            ...
                            @if ($parent != null)
                                <div @class([
                                    ...
                                    $sender = $message?->ownedBy($this->auth)
                                        ? __('filament-wirechat::chat.labels.you')
                                        : ($message->sendable?->wirechat_name ?? __('filament-wirechat::chat.labels.user'));

                                    $receiver = $parent?->ownedBy($this->auth)
                                        ? __('filament-wirechat::chat.labels.you')
                                        : ($parent->sendable?->wirechat_name ?? __('filament-wirechat::chat.labels.user'));
                                    @endphp

                                    <h6 class="text-xs text-gray-500 dark:text-gray-300 px-2">
                                        @if ($parent?->ownedBy($this->auth) && $message?->ownedBy($this->auth))
                                            {{ __('filament-wirechat::chat.labels.you_replied_to_yourself') }}
                                        @elseif ($parent?->ownedBy($this->auth))
                                            {{ __('filament-wirechat::chat.labels.participant_replied_to_you', ['sender' => $sender]) }}
                                        @elseif ($message?->ownedBy($parent->sendable))
                                            {{ __('filament-wirechat::chat.labels.participant_replied_to_themself', ['sender' => $sender]) }}
                                        @else
                                            {{ __('filament-wirechat::chat.labels.participant_replied_other_participant', ['sender' => $sender, 'receiver' => $receiver]) }}
                                        @endif
                                    </h6>
                                    ...
                                    <div @class([
                                        ...
                                        <p
                                            ...
                                            {{ $parent?->body != '' ? $parent?->body : ($parent->hasAttachment() ?  __('filament-wirechat::chat.labels.attachment') : '') }}
                                        </p>
                                    </div>
                            ...
                            <div @class([
                                ...
                                <div dusk="message_actions" @class([ 'my-auto flex  w-auto  items-center gap-2', 'order-1' => !$belongsToAuth, ])>
                                    ...
                                    <x-filament-wirechat::dropdown class="w-40" align="{{ $belongsToAuth ? 'right' : 'left' }}"
                                        width="48">
                                        <x-slot name="trigger">
                                            {{-- Dots --}}
                                        ...
                                        <x-slot name="content">
                                            ...
                                            @if (($message->ownedBy($this->auth)|| ($authParticipant->isAdmin() && $isGroup)))
                                                <button dusk="delete_message_for_everyone" wire:click="deleteForEveryone('{{ encrypt($message->id) }}')"
                                                    wire:confirm="{{ __('filament-wirechat::chat.actions.delete_for_everyone.confirmation_message') }}" class="w-full text-start">
                                                    <x-filament-wirechat::dropdown-link>
                                                        @lang('filament-wirechat::chat.actions.delete_for_everyone.label')
                                                    </x-filament-wirechat::dropdown-link>
                                                </button>
                                            @endif
                                            ...
                                            <button dusk="delete_message_for_me" wire:click="deleteForMe('{{ encrypt($message->id) }}')"
                                                wire:confirm="{{ __('filament-wirechat::chat.actions.delete_for_me.confirmation_message') }}" class="w-full text-start">
                                                <x-filament-wirechat::dropdown-link>
                                                    @lang('filament-wirechat::chat.actions.delete_for_me.label')
                                                </x-filament-wirechat::dropdown-link>
                                            </button>
                                            @endif
                                            ...
                                            <button dusk="reply_to_message_button" wire:click="setReply('{{ encrypt($message->id) }}')"class="w-full text-start">
                                                <x-filament-wirechat::dropdown-link>
                                                    @lang('filament-wirechat::chat.actions.reply.label')
                                                </x-filament-wirechat::dropdown-link>
                                            </button>


                                        </x-slot>
                                    </x-filament-wirechat::dropdown>
                                ...
                                <div class="flex flex-col gap-2 max-w-[95%]  relative">
                                    ...
                                    @if ($attachment)
                                        ...
                                        @if (str()->startsWith($attachment->mime_type, 'application/'))
                                            @include('filament-wirechat::livewire.chat.partials.file', [ 'attachment' => $attachment ])
                                        @endif

                                        {{-- Attachemnt is Video/ --}}
                                        @if (str()->startsWith($attachment->mime_type, 'video/'))
                                            <x-filament-wirechat::video height="max-h-[400px]" :cover="false" source="{{ $attachment?->url }}" />
                                        @endif

                                        {{-- Attachemnt is image/ --}}
                                        @if (str()->startsWith($attachment->mime_type, 'image/'))
                                            @include('filament-wirechat::livewire.chat.partials.image', [ 'previousMessage' => $previousMessage, 'message' => $message, 'nextMessage' => $nextMessage, 'belongsToAuth' => $belongsToAuth, 'attachment' => $attachment ])
                                        @endif
                                    @endif
                                    ...
                                    @include('filament-wirechat::livewire.chat.partials.message', [ 'previousMessage' => $previousMessage, 'message' => $message, 'nextMessage' => $nextMessage, 'belongsToAuth' => $belongsToAuth, 'isGroup' => $isGroup, 'attachment' => $attachment])
                                    @endif
