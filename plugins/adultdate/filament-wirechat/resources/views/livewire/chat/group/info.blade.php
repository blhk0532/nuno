<div id="group-info-modal" class="bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] dark:text-white h-full flex flex-col">
    ...
    <section class="cursor-pointer flex gap-4 z-10  items-center p-5 sticky top-0 bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] dark:text-white border-b border-[var(--wc-light-border)] dark:border-[var(--wc-dark-border)]">
        ...
        <h3>{{__('filament-wirechat::chat.group.info.heading.label')}}</h3>
    </section>
    {{-- Details --}}
    <header>
        ...
        @if ($authIsAdminInGroup || $group?->allowsMembersToEditGroupInfo())
            <div @dusk="edit_group_information_section" class="flex  flex-col items-center gap-5 py-5  px-4    ">
                ...
                <section class="mx-auto items-center justify-center grid">
                    <div @dusk="edit_avatar_label" class="relative  h-32 w-32 overflow-clip mx-auto rounded-full">
                        ...
                        <label wire:target="photo" wire:loading.class="cursor-not-allowed" for="photo"
                            ...
                            <x-filament-wirechat::avatar wire:loading.class="cursor-not-allowed" group="{{ $isGroup }}"
                        ...
                        @if (empty($cover_url))
                            ...
                            <label wire:target="photo" wire:loading.class="cursor-not-allowed"
                                ...
                                class=" cursor-pointer bottom-0 inset-x-0 bg-[var(--wc-light-secondary)]/40 hover:bg-[var(--wc-light-secondary)]/80 dark:bg-[var(--wc-dark-secondary)]/40 dark:hover:bg-[var(--wc-dark-accent)]/80  transition-colors text-gray-600 dark:text-gray-300 flex items-center justify-center  absolute ">
                        ...
                        @else
                            <button type="button" wire:target="photo" wire:loading.attr="disabled"
                                class="disabled:cursor-not-allowed bottom-0 inset-x-0 bg-[var(--wc-light-secondary)]/40 hover:bg-[var(--wc-light-secondary)]/80 m-0 p-0 border-0  dark:bg-[var(--wc-dark-secondary)]/40  dark:hover:bg-[var(--wc-dark-accent)]/80 transition-colors  text-red-800 dark:text-red-400 flex items-center justify-center  absolute "
                ...
                <div class="space-y-3 grid  overflow-x-hidden">
                    ...
                    {{-- Members count --}}
                    <p class="mx-auto">  {{ __('filament-wirechat::chat.group.info.labels.members') }} {{  $totalParticipants }} </p>
                        ...
                        <span class="col-span-11">
                            <div x-show="!editing">
                                @if (empty($description))
                                    <p class="text-sm" style="color: var(--wirechat-primary-color)">{{ __('filament-wirechat::chat.group.info.labels.add_description') }}  </p>
                                @else
        ...
        @else
            ...
            <div @dusk="non_editable_group_information_section" class="flex  flex-col items-center gap-5 py-5 px-4  ">
                <x-filament-wirechat::avatar :src="$cover_url" class=" h-32 w-32 mx-auto" />
    ...
    <x-filament-wirechat::divider />

    {{-- Members section --}}
    <section class="my-4 text-left space-y-3">
        {{-- Actiion button to trigger opening members  modal --}}
        <x-filament-wirechat::actions.open-modal component="filament-wirechat.chat.group.members"
            conversation="{{ $conversation?->id }}" widget="{{ $this->isWidget() }}" panel="{{ $this->panelId() }}">
            {{-- Members count --}}
            <button class="cursor-pointer flex w-full justify-between items-center px-8 focus:outline-hidden ">
                <span class="text-gray-600 dark:text-gray-300">{{ __('filament-wirechat::chat.group.info.labels.members') }}  {{ $totalParticipants }}</span>
                {{-- Search icon --}}
                <span>
        ...
        @if ($authIsAdminInGroup || $group?->allowsMembersToAddOthers())
            <x-filament-wirechat::actions.open-modal component="wirechat.chat.group.add-members"
                ...
                <button @dusk="open_add_members_modal_button"
                    ...
                    <span>{{ __('filament-wirechat::chat.group.info.actions.add_members.label') }}</span>
                </button>
            </x-filament-wirechat::actions.open-modal>
        @endif
    ...
    <footer class="flex flex-col justify-start w-full">
        ...
        @if ($authIsOwner)
            ...
            {{-- Delete group --}}
            <button wire:confirm="{{ __('filament-wirechat::chat.group.info.actions.delete_group.confirmation_message') }}" wire:click="deleteGroup"
                ...
                <div class="flex gap-3 items-center ">
                    ...
                    <span>{{ __('filament-wirechat::chat.group.info.actions.delete_group.label') }}</span>
                </div>

                <p class="dark:text-white/60 text-sm text-gray-600/80">@lang('filament-wirechat::chat.group.info.actions.delete_group.helper_text')</p>
            </button>
            {{-- Permissions --}}
            <div>

                <x-filament-wirechat::actions.open-chat-drawer component='filament-wirechat.chat.group.permissions'
                    ...
                    <button
                        ...
                        <div class="flex gap-3 items-center ">
                            ...
                            <span>@lang('filament-wirechat::chat.group.info.actions.group_permissions.label')</span>
                        </div>
                    </button>
        ...
        @else
        {{-- Exit Group --}}
            <button wire:confirm="{{ __('filament-wirechat::chat.group.info.actions.exit_group.confirmation_message') }}" wire:click="exitConversation"
                ...
                <span>@lang('filament-wirechat::chat.group.info.actions.exit_group.label')</span>
            </button>
        @endif
