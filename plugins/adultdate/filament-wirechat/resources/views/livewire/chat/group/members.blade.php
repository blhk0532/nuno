...
<div x-ref="members"
    ...
    <header class=" sticky top-0 bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] z-10 p-2">
        <div class="flex items-center justify-center pb-2">
            ...
            <h3 class=" mx-auto font-semibold ">{{__('filament-wirechat::chat.group.members.heading.label')}} </h3>
        ...
        <section class="flex flex-wrap items-center px-0 border-b dark:border-[var(--wc-dark-secondary)]">
            <input type="search" id="users-search-field" wire:model.live.debounce='search' autocomplete="off"
                placeholder="{{__('filament-wirechat::chat.group.members.inputs.search.placeholder')}}"
    ...
    <div class="relative w-full p-2 ">
        ...
        <section class="my-4 grid">
                    ...
                    @foreach ($participants as $key => $participant)
                        ...
                        <li x-data="{ open: false }" x-ref="button" @click="open = ! open" x-init="$watch('open', value => {
                            ...
                            <label class="flex cursor-pointer gap-2 items-center w-full">
                                <x-filament-wirechat::avatar src="{{ $participant->participantable->wirechat_avatar_url }}"
                                    class="w-10 h-10" />
                                ...
                                <div class="grid grid-cols-12 w-full ">
                                    <h6 @class(['transition-all truncate group-hover:underline col-span-10' ])>
                                        ...
                                        <span  style="background-color: var(--wirechat-primary-color);" class=" flex items-center col-span-2 text-white text-xs font-medium ml-auto px-2.5 py-px rounded-sm ">
                                            {{$participant->isOwner()? __('filament-wirechat::chat.group.members.labels.owner'): __('filament-wirechat::chat.group.members.labels.admin')}}
                                        </span>
                                        @endif
                                ...
                                <div x-show="open" x-anchor.bottom-end="$refs.button"
                                    ...
                                    <x-filament-wirechat::dropdown-button wire:click="sendMessage('{{ $participant->id }}')"
                                        class="truncate ">
                                        @if ($loopParticipantIsAuth)

                                        {{__('filament-wirechat::chat.group.members.actions.send_message_to_yourself.label')}}
                                        @else

                                        {{__('filament-wirechat::chat.group.members.actions.send_message_to_member.label',['member'=>$participant->participantable?->wirechat_name ])}}
                                        @endif
                                    </x-filament-wirechat::dropdown-button>
                                        ...
                                        @if ($authIsOwner && !$loopParticipantIsAuth)
                                            @if ($participant->isAdmin())
                                                <x-filament-wirechat::dropdown-button
                                                    ...
                                                    wire:confirm="{{__('filament-wirechat::chat.group.members.actions.dismiss_admin.confirmation_message',['member'=>$participant->participantable?->wirechat_name])}}"
                                                    class="  ">
                                                    {{__('filament-wirechat::chat.group.members.actions.dismiss_admin.label')}}
                                                </x-filament-wirechat::dropdown-button>
                                            @else
                                                <x-filament-wirechat::dropdown-button
                                                    wire:click="makeAdmin('{{ $participant->id }}')"
                                                    wire:confirm="{{__('filament-wirechat::chat.group.members.actions.make_admin.confirmation_message',['member'=>$participant->participantable?->wirechat_name])}}"
                                                    class=" ">
                                                    {{__('filament-wirechat::chat.group.members.actions.make_admin.label')}}
                                                </x-filament-wirechat::dropdown-button>
                                            @endif
                                        @endif
                                            ...
                                            <x-filament-wirechat::dropdown-button
                                                ...
                                                wire:confirm="{{__('filament-wirechat::chat.group.members.actions.remove_from_group.confirmation_message',['member'=>$participant->participantable?->wirechat_name])}}"
                                                class="text-red-500 ">
                                                {{__('filament-wirechat::chat.group.members.actions.remove_from_group.label')}}
                                            </x-filament-wirechat::dropdown-button>
                                            @endif
                ...
                @if ($canLoadMore)
                    <section class="w-full justify-center flex my-3">
                        <button dusk="loadMoreButton" @click="$wire.loadMore()"
                            ...
                            {{__('filament-wirechat::chat.group.members.actions.load_more.label')}}
                        </button>
                    </section>
                @endif
            ...
            <span class="m-auto">{{__('filament-wirechat::chat.group.members.labels.no_members_found')}}</span>
            @endif
