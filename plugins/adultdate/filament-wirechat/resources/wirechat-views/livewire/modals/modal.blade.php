        ...
        window.WirechatModal = () => {
            return {
                ...
                closingModal(eventName) {
                    ...
                    const activeComponent = components[this.activeModalComponent];
                ...
                closeModal(force = false, skipPreviousModals = 0, destroySkipped = false) {
                    ...
                    if (this.dispatchCloseEvent === true && this.activeModalComponent) {
                        const components = this.$wire.get('components') || {};
                        const activeComponent = components[this.activeModalComponent];
                        if (activeComponent && activeComponent.name) {
                ...
                init() {
                    ...
                    this.listeners.push(
                        Livewire.on('openWirechatModal', (component, arguments, modalAttributes) => {
                            this.$wire.openWirechatModal(component, arguments, modalAttributes);
                        })
                    );
    ...
    <div x-data="WirechatModal()" x-on:close.stop="setShowPropertyTo(false)"
        ...
        <div class="flex items-end  justify-center min-h-screen px-4 pt-4 pb-10 text-center sm:block sm:p-0">
            ...
            <div x-show="show && showActiveModalComponent"
                ...
                class="inline-block align-bottom wc-scrollbar-theme rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle w-full sm:max-w-lg {{ $sidebarOpen ? 'wirechat-sidebar-modal' : 'wirechat-widget-modal' }} bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] dark:text-white"
