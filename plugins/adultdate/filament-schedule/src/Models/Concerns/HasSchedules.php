...
trait HasSchedules
{
    ...
    public function isAvailableAt(string $date, string $startTime, string $endTime, ?Collection $schedules = null): bool
    {
        ...
        $schedules = $schedules ?? \Adultdate\Schedule\Models\Schedule::where('schedulable_type', get_class($this))
     ...
     */
    protected function scheduleBlocksTime(\Adultdate\Schedule\Models\Schedule $schedule, string $date, string $startTime, string $endTime): bool
    {
     ...
     */
    protected function recurringScheduleBlocksTime(\Adultdate\Schedule\Models\Schedule $schedule, string $date, string $startTime, string $endTime, int $bufferMinutes = 0): bool
    {
     ...
     */
    protected function shouldCreateRecurringInstance(\Adultdate\Schedule\Models\Schedule $schedule, \Carbon\CarbonInterface $date): bool
    {
    ...
    public function getAvailableSlots(
        ...
        $schedules = \Adultdate\Schedule\Models\Schedule::where('schedulable_type', get_class($this))
            ->where('schedulable_id', $this->getKey())
            ->active()
            ->forDate($date)
            ->with('periods')
