...
>
    @if ($hasChildren)
        ...
        >
            <x-slot name="icon">
                <x-filament::icon icon="heroicon-o-minus" class="w-4 h-4" x-show="isNodeExpanded('{{ $key }}')" />
                <x-filament::icon icon="heroicon-o-plus" class="w-4 h-4" x-show="!isNodeExpanded('{{ $key }}')" />
            </x-slot>
        </x-filament::icon-button>
    @endif
